...
@Configuration
@EnableConfigurationProperties({TokenSvcConfigProperties.class, DokuConfigProperties.class})
public class GlobalConfig {
    @Bean
    public Symmetric symmetric(DokuConfigProperties dokuConfig) {
        return new Symmetric(PaymentServiceConstant.ALGORITHM_HMAC_SHA512, dokuConfig.api().key());
    }
